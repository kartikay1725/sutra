...
export default function KnowledgeGraphGlobalPage() {
    ...
    <AppShell>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
        <PageHead
          ...
          sub="Explore semantic dependencies between files, functions, classes, tasks, and changes across repositories."
        />
        ...
        <div style={{ margin: "24px 0", display: "flex", gap: 12 }}>
          <div style={{ position: "relative", flex: 1 }}>
            ...
            <input
              type="text"
              placeholder="Search repositories for knowledge graph..."
              value={search}
        ...
        {loading ? (
          ...
          <Card style={{ padding: 40, textAlign: "center" }}>
            <I.Database size={32} style={{ color: "var(--muted)", margin: "0 auto 12px", opacity: 0.6 }} />
            <div style={{ fontSize: 16, fontWeight: 600, color: "var(--fg)" }}>No repositories found</div>
            <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 4 }}>
              Create or import a repository to view its semantic knowledge graph.
            </div>
          </Card>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
            {filtered.map((repo) => (
              <Link
                key={repo.id}
                href={`/repositories/${encodeURIComponent(repo.name)}/knowledge-graph`}
